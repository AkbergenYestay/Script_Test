...
default_args = {
    "owner": "AlmasA",
    "email": ["AlmasA@halykbank.kz", "EstayA@halykbank.kz", "QuanysQo@halykbank.kz", "AIYGERIMIB@halykbank.kz", "DaniyarMussa@halykbank.kz"],
...
def taskflow():
    ...
    @task(
        task_id="fetch_dataframe_and_send_email",
    )
    def fetch_df(oracle_hook: OracleHook) -> pd.DataFrame:
        print("This thing is executed during DAG parsing")
        select = """
...
ORDER BY wf_starttime, wf_name
        ...
        dataframe.columns = ["WORKFLOW_ID", "WF_NAME", "WF_STARTTIME", "WF_ENDTIME", "WF_STATUS", "INST_NAME", "INST_STATUS", "INST_STARTTIME", "INST_ENDTIME",  "WF_SCHEDTIME"]
        html_result = dataframe.to_html(index=False)
        if data:
            send_email(
                to=[
                    "AlmasA@halykbank.kz", "EstayA@halykbank.kz", "QuanysQo@halykbank.kz", "AIYGERIMIB@halykbank.kz", "DaniyarMussa@halykbank.kz"
                ],
